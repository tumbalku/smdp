...
import { EmploymentStatusOption, ProfessionGroupOption } from "../types";
...
type DataType = "STATUS" | "GROUP" | "PROFESSION" | "POSITION";
...
export function ManageCategoriesModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/admin/employment-categories", {
        method: "POST",
      ...
      setSuccess(`Data "${name.trim()}" berhasil ditambahkan.`);
      setName("");
          ...
          <form onSubmit={handleSubmit} className="bg-muted/30 border border-border p-4 rounded-xl space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                <Select value={type} onValueChange={(val) => setType((val || "STATUS") as DataType)}>
                        ...
                        {status.groups && status.groups.length > 0 ? (
                          <div className="pl-6 space-y-1 border-l border-border/80 ml-1.5 pt-0.5 pb-0.5">
                            {status.groups.map((group) => (
                              <div key={group.id} className="text-xs text-muted-foreground flex items-center gap-1">
                        ...
                        {prof.positions && prof.positions.length > 0 ? (
                          <div className="pl-6 space-y-1 border-l border-border/80 ml-1.5 pt-0.5 pb-0.5">
                            {prof.positions.map((pos) => (
                              <div key={pos.id} className="text-xs text-muted-foreground flex items-center gap-1">
                                <span className="w-1.5 h-1.5 bg-[#6c63ff] rounded-full shrink-0" />
                                <span>{pos.name}</span>
                              </div>
                            ))}
